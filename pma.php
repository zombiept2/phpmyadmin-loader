...
/**
 * Please copy this file into your webserver root and open it with a browser. The setup wizard checks the dependency, downloads the newest phpMyAdmin version, unpacks it and redirects to the phpMyAdmin interface.
 */
...
class Setup 
{
	...
	*/
	static public function checkDependencies() 
	{
		...
		{
			$error.='PHP 5.5.0 is required. Please ask your server administrator to update PHP to version 5.5.0 or higher.<br/>';
		}
		...
		if (!empty($missingDependencies)) {
			$error .= 'The following PHP modules are required to use phpMyAdmin:<br/>';
		}
	...
	*/
	...
	/**
	* Performs the phpMyAdmin install.
	* @return string with error messages
	*/
	static public function install() 
	{
		...
		if (file_exists('./config.sample.inc.php')) 
		{
			return 'There seems to already be an phpMyAdmin installation. - You cannot use this script to update existing installation.';
		}

		// downloading latest release
		if (!file_exists('pma.zip')) 
		{
			if ($dev)
			{
				$error .= Setup::getFile('https://files.phpmyadmin.net/phpMyAdmin/5.0.2/phpMyAdmin-5.0.2-all-languages.zip','pma.zip');
			}
			else
			{
				$error .= Setup::getFile('https://files.phpmyadmin.net/phpMyAdmin/5.0.2/phpMyAdmin-5.0.2-all-languages.zip','pma.zip');
		...
		// unpacking into phpMyAdmin folder
		$zip = new ZipArchive;
		$res = $zip->open('pma.zip');
		if ($res == true) 
		{
			// Extract it
			$zip->extractTo(dirname(__FILE__));
			$zip->close();
            Setup::recursiveCopy(dirname(__FILE__) . '/phpMyAdmin-5.0.2-all-languages', dirname(__FILE__));
            Setup::recursiveRemoveDirectory(dirname(__FILE__) . '/phpMyAdmin-5.0.2-all-languages');
		} 
		else 
		{
			$error .= 'unzip of phpMyAdmin source file failed.<br />';
		}

		// deleting zip file
		$result = @unlink('pma.zip');
		if ($result == false) 
		{
			$error .= 'deleting of pma.zip failed.<br />';
		}
		return($error);
	}
	...
	*/
	static public function getFile($url,$path) 
	{
		...
		if ($data == false)
		{
			$error .= 'download of phpMyAdmin source file failed.<br />'.$curlError;
		}
	...
	static public function showHeader() 
	{
		...
		<html lang="en">
			<head>
				...
				<link rel="icon" href="https://www.phpmyadmin.net/static/favicon.ico">
				<title>phpMyAdmin Loader</title>
					...
					<div class="card text-center">
						<div class="card-header">
							<a href="https://www.phpmyadmin.net" target="_blank">
								<img src="https://www.phpmyadmin.net/static/images/logo.png?2ccd9afd557e" alt="phpMyAdmin" class="logo" />
							</a>
							<h1>phpMyAdmin Loader</h1>
						</div>
						<div class="card-body">
		');
	}
	...
	*/
	...
	{
		$txt = '<p>Welcome to the phpMyAdmin Loader Wizard.</p><p>This wizard will check the phpMyAdmin dependencies, download the latest version of phpMyAdmin and redirect to the installer in a few simple steps.</p>';
	...
	*/
	static public function showCheckDependencies() 
	{
		...
		if ($error=='') 
		{
			$txt='<p>All phpMyAdmin dependencies found</p>';
	...
	*/
	static public function showInstall() 
	{
		...
		if ($error == '') 
		{
			$txt='<p>phpMyAdmin is now loaded</p>';
			Setup::showContent('Success',$txt,3);
		}
		else
		{
			$txt='<p>phpMyAdmin is NOT loaded<br />'.$error.'</p>';
